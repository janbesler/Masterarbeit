...
The model architecture consists of the time2vec embeddings created by \cite{time2vec}, an auto-correlation mechanism by \cite{autoformer} that replaces the attention mechanism in a vanilla transformer and a sparse-attention mechanism that reduces the complexity calculation published by \cite{Informer} that resides in a different head to the previous attention mechanism. The two attention modules are not used in a row after one another, but separately. This is due to the inner workings of both attention mechanisms, since both filter out irrelevant parts and with too much filtering more information is lost than initially desired and the weights are converging towards zero too often, leaving fewer elements in the weight than desired. Hence in the first step there are going to be four models, only one attention mechanism per model and each mechanism as a probabilistic and a deterministic variant, while the deterministic variants have a probabilistic layer as second last layer. In a later attempt the goal is to combine the best working models from both attention mechanisms into one coherent structure, where each odd weight update is mechanism A and each even weight update is mechanism B. \par
...
\begin{itemize}
    \item deterministic since no uncertainty is required
    \item easier for computer to understand time when used as sinus function
    \item 
\end{itemize}
...
There exist two versions of the same eFormer with sparse attention, in one case the variance is calculated already in the attention mechanism and from there kept and updated alongside the mean, while the second case only calculates with the mean until the very end where the variance is calculated just after the softmax layer and before the forecast output is produced. The idea is to investigate whether there are benefits to keep track of the variance for the majority of the time and risk the higher computational costs which are expected due to twice the storage and calculation of an additional matrix. It is expected that the approach with the variance calculation early on yields higher accuracy, due to the set of additional trainable variables. And that the variance is more nuanced in some areas which in return can be also interpreted further and result in more information gained. It can for example be advantageous to know for during which time the variance is smaller or larger to imply policy predictions from that knowledge. 

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.5\linewidth]{graphs/eFormer_probSparse.png}
